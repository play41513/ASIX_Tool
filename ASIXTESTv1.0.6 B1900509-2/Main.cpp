...
{
	AnsiString SS;
	AnsiString LOGFilePath = "C:\\ASMP\\log\\ASIXTool\\"+edtSetWorkOrderNumber->Caption+"\\"+edtWorkStation->Caption;
	if(bAction)
	{  //創LOG資料夾
		if(!FileExists(LOGFilePath.c_str())){
			...
			SS = "C:\\ASMP\\log\\ASIXTool\\"+edtSetWorkOrderNumber->Caption;
			_mkdir(SS.c_str());
...
bool TfrmMain::CheckDiskName()
{
	...
	for(int i =3;i<=25;i++)
	{
		if((DiskInfo&(0x01<<i))!=0)
		{
			...
			{
				LogDiskPath = DiskName+"\\ASMP\\log\\ASIXTool\\"+edtSetWorkOrderNumber->Caption+"\\"+edtWorkStation->Caption;
				if(!FileExists(LogDiskPath.c_str())){
					...
					_mkdir( SS.c_str());
					SS = DiskName + "\\ASMP\\log\\ASIXTool";
					_mkdir( SS.c_str());
					SS = DiskName + "\\ASMP\\log\\ASIXTool\\"+edtSetWorkOrderNumber->Caption;
					_mkdir(SS.c_str());
