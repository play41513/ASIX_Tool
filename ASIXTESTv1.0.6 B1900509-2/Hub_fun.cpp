		...
		do
		{
			...
			if(!frmMain->TListInitLAN->Text.Pos(lanname))
			{
				...
				else
				{
					...
					{
						if(frmMain->bBurnPASS && frmMain->Test_Step != GET_DUT)
						{
							frmMain->current_mac->Caption = frmMain->new_mac->Caption;
						}
...
void Port_test::IntiNetwordCard()
{
	...
	if(ERROR_SUCCESS==GetAdaptersInfo((IP_ADAPTER_INFO*)pbBuf1, &ulLen1))
	{
		...
		do
		{
			if(!AnsiString(pipai->Description).Pos("ASIX") && !AnsiString(pipai->Description).Pos("AX88179") && !AnsiString(pipai->Description).Pos("AX88772"))
