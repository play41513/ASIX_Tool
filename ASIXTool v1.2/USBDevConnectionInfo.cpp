...
bool USBDevConnectionInfo::GetDevUSB(AnsiString VID,AnsiString PID)
{
	...
	for(int j=0;j<TListUSBDEV_CONN_INFO->Count;j++)
	{
		...
		if (strstr(TListUSBDEV_CONN_INFO->Strings[j].t_str(),("VID :"+VID).c_str()))//確認裝置VID
		{
			j++;
			b = TListUSBDEV_CONN_INFO->Strings[j];
			if (strstr(TListUSBDEV_CONN_INFO->Strings[j].t_str(),("PID :"+PID).c_str()))//確認裝置PID
			{
				while(j+1!=TListUSBDEV_CONN_INFO->Count-1)
				{
					j++;
					c = TListUSBDEV_CONN_INFO->Strings[j];
					if(TListUSBDEV_CONN_INFO->Strings[j].Pos("wMaxPacketSize "))
					{
						if(TListUSBDEV_CONN_INFO->Strings[j].SubString(20,4).ToInt()==400)
						{
							delete TListUSBDEV_CONN_INFO;
							return true;
						}
						else if(TListUSBDEV_CONN_INFO->Strings[j].SubString(20,4).ToInt()==200)
						{
							delete TListUSBDEV_CONN_INFO;
							return false;
						}
...
AnsiString USBDevConnectionInfo::DevNameGetVPID(AnsiString DevName)
{
	if(DevName.Pos("#"))
		DevName = DevName.SubString(1,DevName.Pos("#")-1).Trim();
	HDEVINFO hDevInfo;
	SP_DEVINFO_DATA DeviceInfoData;
	DWORD i,j;
	AnsiString SS,USBPath;
			...
			&ifdata))                 //   設備介面資訊
			{
					...
					&spdd))               //   不需額外的設備描述
				{
					try
					{
						...
						SS=ch;
						//DEBUG(SS);
						if(SS.Pos("vid_17e9")||SS.Pos("vid_0b95")||SS.Pos("vid_0bda"))
						{
							if(SS.Pos("vid_0bda"))
							{
								...
								if(CheckReltekName(SS))
								{
									GlobalFree(pDetail);
									SetupDiDestroyDeviceInfoList(hDevInfo);
									return SS.SubString(1,17).UpperCase();
								}
							}
							else
							{
	...
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, temppath.c_str(), 0, KEY_READ,&hSoftKey) == ERROR_SUCCESS)
	{
		...
		if (lResult == ERROR_SUCCESS)
		{
			...
			if(lResult == ERROR_SUCCESS)
			{
				...
				if(SS.UpperCase().Pos("REALTEK USB"))
					bl = true;
			}
